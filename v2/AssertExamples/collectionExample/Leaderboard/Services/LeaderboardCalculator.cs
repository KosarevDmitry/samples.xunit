...
public class LeaderboardCalculator : ILeaderboardCalculator
{
    ...
    {
        var sortedUsersWithScores = usersWithScores.OrderByDescending(x => x.Score);
        List<UserWithPlace> result = new();
        int[] mins =
        {
            leaderboardMinScores.FirstPlaceMinScore, leaderboardMinScores.SecondPlaceMinScore,
            leaderboardMinScores.ThirdPlaceMinScore
        };
        int start = 0;
        foreach (IUserWithScore item in sortedUsersWithScores)
        {
            bool isAdded = false;
            if (start < mins.Length)
            {
                int i = start;
                for (; i < mins.Length; i++)
                {
                    if (item.Score >= mins[i])
                    {
                        result.Add(new UserWithPlace(userId: item.UserId, place: i + 1));
                        i++;
                        isAdded = true;
                        break;
                    }
            ...
            if (start >= mins.Length && !isAdded)
                result.Add(new UserWithPlace(userId: item.UserId, place: ++start));
        }
